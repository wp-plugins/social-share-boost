...
function ssb_in_content($content)
{
    ...
    if (in_array($this_id, $hide_arr))
        return $content;
    if(  (get_post_type( $this_id )=="page" && $ssb['show_pages']) or  (get_post_type( $this_id )=="post" && $ssb['show_posts'])   )
    {
        // print_r($ssb);
        if($ssb['show_top']==1)
            $content = ssb_output(1, $ssb,0).$content;
        if($ssb['show_bottom']==1)
...
function ssb_notice() {
    ...
    if ( ! get_user_meta($user_id, 'ssb_notice_ignore') ) {
        if( $pagenow != 'admin.php' && $_GET['page'] != 'social-share-boost' ) {
            echo '<div class="updated"><p>';
            printf(__('<a href="%1$s" style="float: right;">Dismiss</a>'), '?ssb_notice_ignore=0');
            echo '<strong>Social Share Boost has gone through a major overhaul in version 3.0! ';
...
function ssb_notice_ignore_func() {
    global $current_user, $pagenow;

    $user_id = $current_user->ID;
    
    if( $pagenow == 'admin.php' && $_GET['page'] == 'social-share-boost' && isset($_GET['whatsnew']) && $_GET['whatsnew']=='true' ) {
        add_user_meta($user_id, 'ssb_notice_ignore', 'true', true);
    }

    if ( isset($_GET['ssb_notice_ignore']) && '0' == $_GET['ssb_notice_ignore'] ) {
        add_user_meta($user_id, 'ssb_notice_ignore', 'true', true);
    }

    if ( isset($_GET['ssb_whatsnewbox']) && '0' == $_GET['ssb_whatsnewbox'] ) {
        add_user_meta($user_id, 'ssb_whatsnewbox', 'true', true);
    }
}
...
if($ssb['show_shortcode']){add_shortcode( 'ssboost', 'ssb_shortcode' );}
if($ssb['show_excerpt']){add_filter('the_excerpt', 'ssb_in_content');}
if($ssb['show_widget']){add_action( 'widgets_init', 'ssb_widget_reg_func' );}
add_filter('the_content', 'ssb_in_content');
add_action( 'wp_enqueue_scripts', 'ssb_button_scripts' );
add_action('admin_notices', 'ssb_notice');
add_action('admin_init', 'ssb_notice_ignore_func');
